    ...
    <div v-show="showFlag" class="food" ref="food">
      <div class="food-content">
        ...
        <div class="rating">
          ...
          <ratingselect @content-toggle="contentToggle" @ratingtype-select="ratingtypeSelect" :ratings="food.ratings" :desc="desc" :only-content="onlyContent" :select-type="selectType"></ratingselect>
...
export default {
  ...
  methods: {
    show () {
      this.showFlag = true
      this.selectType = ALL
      this.onlyContent = false // 是否只展示有内容的评价
      // 图层打开的时候，使用$nextTick 等DOM渲染成功再去使用BScroll计算高度
      this.$nextTick(() => {
        // 因为页面可能会多次打开，如果第一次初始化scroll
        // 不是第一次的话就重新计算高度即可
        if (!this.scroll) {
    ...
    addFirst (event) {
      ...
      this.$emit('cart-adds', event.target)
